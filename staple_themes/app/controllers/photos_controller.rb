class PhotosController < ApplicationController
    ...
    def create
        if user
            Theme.transaction do
                if @theme
                    #product.lock!
                    if theme.photos.length < 4
                        photo = theme.photos.create!(:photo => params[:file], :photoable_id => theme.id, :user_uuid => current_user.uuid, :uuid => Photo.setUUID, :created_at=>Time.now, :updated_at =>Time.now)
                        if product.sorted
  ...
  def owned_theme
    @theme = params[:id] ? ::Theme.where("uuid = ?", params[:id]).first : nil
