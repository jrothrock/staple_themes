var welcomeScreenFunctions = {
  ...
  importDemoContent: function() {
    ...
    jQuery( '.epsilon-ajax-button' ).click( function( e ) {
      ...
      if ( importThis[ 'plugins' ].length ) {
        jQuery( document ).on( 'epsilon-all-plugins-imported', function() {
          self._importContent( importThis[ 'stencil_options' ], container );
        } );
      } else {
        self._importContent( importThis[ 'stencil_options' ], container );
      }
    ...
    */
    fixHomepage: function() {
        jQuery( 'body' ).on( 'click', '#stencil-fix-homepage', function ( evt ) {
            var button = jQuery(this);
            evt.preventDefault();
            jQuery.ajax({
                type      : "POST",
                data      : { action: 'stencil_set_frontpage', setfrontpage: 1 },
                dataType  : "html",
                url       : stencilWelcomeScreenObject.ajaxurl,
  ...
  _importContent: function( $import, container ) {
    var args = {
      ...
      nonce: stencilWelcomeScreenObject.ajax_nonce,
      args: $import
    };
  ...
  dismissAction: function() {
    ...
    jQuery( '.required-action-button' ).click( function() {
      args = {
        action: [ 'stencil_Welcome_Screen', 'handle_required_action' ],
        nonce: stencilWelcomeScreenObject.ajax_nonce,
        args: {
    ...
    jQuery( '.stencil-recommended-plugin-button' ).click( function() {
      args = {
        action: [ 'stencil_Welcome_Screen', 'handle_required_plugin' ],
        nonce: stencilWelcomeScreenObject.ajax_nonce,
        args: {
          'do': jQuery( this ).attr( 'data-action' ),
          'id': jQuery( this ).attr( 'id' )
  ...
  activatePlugin: function() {
    ...
    jQuery( activateButtonSlug ).on( 'click', function( e ) {
      ...
      jQuery.ajax( {
        beforeSend: function() {
          self.replaceWith( '<a class="button updating-message">' + stencilWelcomeScreenObject.activating_string + '...</a>' );
        },
        async: true,
        type: 'GET',
        dataType: 'html',
        ...
        success: function( response ) {
          ...
          if ( ! actions.length ) {
            jQuery( '#plugin-filter' ).append( '<span class"hooray">' + stencilWelcomeScreenObject.no_actions + '</span>' );
          }
    ...
    jQuery( document ).on( 'wp-plugin-install-success', function( response, data ) {
      ...
      if ( activateButton.length && ( jQuery( 'body' ).hasClass( stencilWelcomeScreenObject.body_class ) || jQuery( 'body' ).hasClass( 'wp-customizer' ) ) ) {

        jQuery.ajax( {
          beforeSend: function() {
            activateButton.replaceWith( '<a class="button updating-message">' + stencilWelcomeScreenObject.activating_string + '...</a>' );
          },
          async: true,
          type: 'GET',
          dataType: 'html',
