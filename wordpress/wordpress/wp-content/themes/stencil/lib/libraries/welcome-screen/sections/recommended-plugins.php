...
<div class="feature-section recommended-plugins three-col demo-import-boxed" id="plugin-filter">
	<?php foreach ( $stencil_recommended_plugins as $plugin => $prop ) { ?>
		<?php
		...
		switch ( $active['needs'] ) {
			case 'install':
				...
				$label = __( 'Install', 'stencil' );
				break;
			case 'activate':
				$class = 'activate-now button button-primary';
				$label = __( 'Activate', 'stencil' );
				break;
			case 'deactivate':
				$class = 'deactivate-now button';
				$label = __( 'Deactivate', 'stencil' );
				break;
		}
		...
		<div class="col plugin_box">
			<?php if ( $prop['recommended'] ) : ?>
				<span class="recommended"><?php _e( 'Recommended', 'stencil' ); ?></span>
			<?php endif; ?>
			<img src="<?php echo esc_attr( $icon ); ?>" alt="plugin box image">
			<span
				class="version"><?php echo __( 'Version:', 'stencil' ); ?><?php echo esc_html( $info->version ); ?></span>
			<span
