...
class Epsilon_Section_Recommended_Actions extends WP_Customize_Section {
	...
	public function json() {
		...
		$json['facebook_text']  = esc_html__( 'Facebook', 'stencil' );
		$json['twitter']        = $this->twitter;
		$json['twitter_text']   = esc_html__( 'Twitter', 'stencil' );
		$json['wp_review']      = $this->wp_review;
		$json['wp_review_text'] = esc_html__( 'Review this theme on w.org', 'stencil' );
		if ( $this->wp_review ) {
			...
			if ( ! empty( $v['plugin_slug'] ) ) {
				...
				switch ( $active['needs'] ) {
					case 'install':
						...
						$v['button_label'] = esc_html__( 'Install', 'stencil' );
						break;
					case 'activate':
						$v['button_class'] = 'activate-now button button-primary';
						$v['button_label'] = esc_html__( 'Activate', 'stencil' );
						break;
					case 'update':
						$v['button_class'] = 'update-now button button-primary';
						$v['button_label'] = esc_html__( 'Update', 'stencil' );
						break;
					case 'deactivate':
						$v['button_class'] = 'deactivate-now button';
						$v['button_label'] = esc_html__( 'Deactivate', 'stencil' );
						break;
				}
		...
		foreach ( $this->plugins as $k => $v ) {
			...
			switch ( $active['needs'] ) {
				case 'install':
					...
					$t['button_label'] = esc_html__( 'Install', 'stencil' );
					break;
				case 'activate':
					$t['button_class'] = 'activate-now button button-primary';
					$t['button_label'] = esc_html__( 'Activate', 'stencil' );
					break;
				case 'update':
					$t['button_class'] = 'update-now button button-primary';
					$t['button_label'] = esc_html__( 'Update', 'stencil' );
					break;
				case 'deactivate':
					$t['button_class'] = 'deactivate-now button';
					$t['button_label'] = esc_html__( 'Deactivate', 'stencil' );
					break;
			}
